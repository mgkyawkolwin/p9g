...
import { billTable, configTable, customerTable, paymentTable, reservationTable, roomTable, userTable } from "@/data/orm/drizzle/mysql/schema";
...
export default class ReportRepository extends Repository<Customer, typeof customerTable> implements IReportRepository {
    ...
    async getDailySummaryGuestsRoomsReport(startDate: string, endDate: string): Promise<DailySummaryGuestsRoomsReportRow[]> {
        c.i("getDailySummaryGuestsRoomsReport");
        c.d(startDate);
        c.d(endDate);
    ...
    async getDailySummaryIncomeReport(startDate: string, endDate: string): Promise<DailySummaryIncomeReportRow[]> {
        c.i("getDailySummaryIncomeReport");
        c.d(startDate);
        c.d(endDate);
        const session = await auth();
        if (!session || !session.user) throw new CustomError('Repository cannot get session.');
    ...
    async getDailySummaryPersonReport(startDate: string, endDate: string): Promise<DailySummaryPersonReportRow[]> {
        c.i("getDailySummaryPersonReport");
        c.d(startDate);
        c.d(endDate);
        const session = await auth();
        if (!session || !session.user) throw new CustomError('Repository cannot get session.');
