...
export default class DrizzleQueryObjectTransformer {

  async transform<TResult, TQuery>(query: TQuery): Promise<TResult | null> {

    if(typeof query === 'string'){
      return sql`${query}` as TResult;
    }
    ...
    for (const [key, value] of Object.entries(q)) {
      ...
      if (key.endsWith('From') || key.endsWith('Until')) {
        ...
        const fromValue = q[`${baseKey}From`];
        const toValue = q[`${baseKey}Until`];
   ...
   */
  transformOrderBy<TResult, TQuery>(query: TQuery): TResult {
    if(typeof query === "string")
      return sql`${query}` as TResult;

    const q = query as Record<string, any>;
    return sql`${sql.identifier(q.column)} ${sql.raw(q.direction.toUpperCase())}` as TResult;
  }
}