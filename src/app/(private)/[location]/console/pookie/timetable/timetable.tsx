...
export default function TimeTable() {
    ...
    const columns: ColumnDef<PookieTimeTable>[] = [
        {
            ...
            cell: row => (<CheckboxCustom checked={Boolean(row.getValue())} onCheckedChange={checked => {
                ...
                updateTimeTable(changedItem, location).then(response => {
                    if (response.message) {
    ...
    const handleViewNoDraw = async () => {
        ...
        const res = await getNoDraw(drawDate.getLocalDateAsUTCDate(), location);
        if (res.message) {
            if (res.error) {
                toast(res.message);
                return;
    ...
    const fetchData = () => {
        ...
        if (drawDate) {
            getTimeTable(drawDate.getLocalDateAsUTCDate(), location).then((res) => {
                if (res.message) {
                    toast(res.message);
                }
                        ...
                        <div className="flex flex-col gap-y-2">
                            ...
                            <div className="flex justify-between">
                                ...
                                <ButtonCustom onClick={() => {
                                    ...
                                    generateTimeTable(drawDate, startDate, endDate, location).then((res) => {
                                        if (res.message) {
                                            toast(res.message);
                                        }
                                        if (!res.error && res.data) {
                            ...
                            <div>
                                <div className="flex gap-4">
                                    <ButtonCustom variant="green" onClick={() => {
                                        updateTimeTable(selectedItem, location).then(response => {
                                            if (response.message) {
                                                toast(response.message);
                                            }
                                            if (!response.error) {
