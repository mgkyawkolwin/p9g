...
export async function getDailySummaryRoomOccupancyReport(startDate: string, endDate: string, location: string): Promise<FormState> {
    try {
        c.fs('Actions > getDailySummaryRoomOccupancyReport');
        c.i('Get report by calling api.');

        const response = await fetch(process.env.API_URL + `reports/dailysummaryroomoccupancyreport?startDate=${startDate}&endDate=${endDate}`, {
            method: 'GET',
            headers: {
        ...
        c.fe('Actions > getDailySummaryRoomOccupancyReport');
