          ...
          <div className="flex flex-col gap-4">
            <section aria-label="reportsearch">
              ...
              <ButtonCustom onClick={async () => {
                setIsLoading(true);
                const response = await getDailySummaryIncomeReport(fromDate ? getISODateTimeString(fromDate.toLocaleDateString('sv-SE')) : '', toDate ? getISODateTimeMidNightString(toDate.toLocaleDateString('sv-SE')) : '', reservationType, location);
                setIsLoading(false);
                if(response.message)
