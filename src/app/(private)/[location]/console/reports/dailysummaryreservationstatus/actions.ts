...
export async function getDailySummaryReservationStatusReport(startDate: string, endDate: string, location: string): Promise<FormState> {
    try {
        c.fs('Actions > getDailySummaryReservationStatusReport');
        c.i('Get report by calling api.');

        const response = await fetch(process.env.API_URL + `reports/dailysummaryreservationstatusreport?startDate=${startDate}&endDate=${endDate}`, {
            method: 'GET',
            headers: {
        ...
        c.fe('Actions > getDailySummaryReservationStatusReport');
