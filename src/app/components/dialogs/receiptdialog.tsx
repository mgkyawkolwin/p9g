  ...
  const handlePrint = () => {
    ...
    if (win) {
      ...
      win.document.writeln(
        ...
        `<tr><td>Customer: ${reservation?.customers?.reduce((acc, c) => acc + (acc ? ", " : "") + (c.englishName && c.englishName?.trim() !== "" ? c.englishName : c.name), "")}</td><td style="text-align:right;">Date: ____/____/________</td></tr>` + 
        `<tr><td>Reservation Type: ${reservation?.reservationType}</td><td></td></tr>` +
        `</table>` +
        `<div><br/>${content}</div><br/><br/><br/>` +
        `<table style="cell-padding:5px;border:0;width:100%;">` +
        `<tr><td>____________________</td><td>____________________</td><td>____________________</td><td>____________________</td></tr>` +
        `<tr><td style="text-align:center;">Cashier SIgn</td><td style="text-align:center;">In-charge Sign</td><td style="text-align:center;">Finance Cashier Sign</td><td style="text-align:center;">Accountant Sign</td></tr>` +
        `</table>` +
        `Remark: <br/> ${reservation?.paymentRemark ?? ''}<br/><br/>` +
