...
import { reservationCancel, reservationCheckIn } from "@/app/(private)/[location]/console/checkin/actions"
          ...
          <DialogContent className="">
            ...
            <DialogFooter>
              <ButtonCustom variant={"green"} type="button" onClick={async () => {
                ...
                const response = await reservationCheckIn(checkInId, location);
                toast(response.message);
                setCheckInId('');
                //reload page
                formRef.current?.requestSubmit();
