...
export async function GET(request: NextRequest) {
  try {
    c.fs('GET /api/reports/monthlysummaryreservationstatusreport');
    const session = await auth();
    if (!session?.user) throw new CustomError('Invalid session');
    ...
    const result = await reportService.getMonthlySummaryReservationStatusReport(
      searchParams.year,
      session.user
    );
